...
<div class="space-y-6">
    ...
    <div class="grid gap-4 md:grid-cols-[1fr_240px] items-end mt-6">
        <form method="GET" action="<?php echo e(route('admin.donors.index')); ?>" class="grid gap-3 sm:grid-cols-[1fr_auto] items-end">
            <div>
        ...
        <form method="GET" action="<?php echo e(route('admin.donors.index')); ?>" class="flex flex-wrap items-center gap-3">
            ...
            <select id="filter" name="filter" onchange="this.form.submit()" class="form-field dark:form-field-dark rounded-full px-4 py-3 text-sm">
                <?php $__currentLoopData = ['all' => 'All donors', 'available' => 'Available', 'unavailable' => 'Unavailable']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <select id="per_page" name="per_page" onchange="this.form.submit()" class="form-field dark:form-field-dark rounded-full px-4 py-3 text-sm">
                <?php $__currentLoopData = [12,25,50,100]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $size): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($size); ?>" <?php if(($perPage ?? 12) == $size): echo 'selected'; endif; ?>><?php echo e($size); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </form>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-950/30 dark:text-slate-300 mt-4">
        Showing <?php echo e($donors->firstItem() ?? 0); ?> to <?php echo e($donors->lastItem() ?? 0); ?> of <?php echo e($donors->total()); ?> donors.
    </div>
