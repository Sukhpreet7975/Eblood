    ...
    <div class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                ...
                <div class="mt-6 rounded-[2rem] border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/70">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            ...
                            <div class="mt-2 flex flex-wrap items-center gap-3">
                                <span id="availability-badge" class="inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold {{ $user->available == 'yes' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <form id="availability-form" method="POST" action="{{ route('toggle.availability') }}">
                            @csrf
                            <button id="toggle-availability-btn" type="button" class="rounded-full px-5 py-3 text-sm font-semibold shadow-lg transition {{ $user->available == 'yes' ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-emerald-600 text-white hover:bg-emerald-700' }}">
                                {{ $user->available == 'yes' ? 'Set as Unavailable' : 'Set as Available' }}
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-3">
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm dark:bg-slate-950">
                            ...
                            <p class="mt-2 text-lg font-bold text-slate-900 dark:text-white">{{ $user->blood_group ?? 'Not added' }}</p>
                        </div>
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm dark:bg-slate-950">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-300">City</p>
                            <p class="mt-2 text-lg font-bold text-slate-900 dark:text-white">{{ $user->city ?? 'Not added' }}</p>
                        </div>
                        <div class="rounded-2xl bg-white px-4 py-3 shadow-sm dark:bg-slate-950">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-300">Phone</p>
                            <p class="mt-2 text-lg font-bold text-slate-900 dark:text-white">{{ $user->phone ?? 'Not added' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="card-panel dark:card-panel-dark">
                ...
                <a href="{{ route('profile') }}" class="mt-4 inline-flex items-center rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">
                    Go to Profile
                </a>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        toggleBtn.addEventListener('click', async function () {
            ...
            try {
                const response = await fetch('{{ route('toggle.availability') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrf,
