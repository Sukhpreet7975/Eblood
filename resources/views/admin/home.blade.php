                ...
                @forelse($latestDonors as $donor)
                    <div class="card-panel dark:card-panel-dark">
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700 dark:bg-emerald-900 dark:text-emerald-200">{{ ucfirst($donor->blood_group ?? 'N/A') }}</span>
                        </div>
                        <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Joined {{ optional($donor->created_at)->diffForHumans() }}</p>
                    </div>
                @empty
                    ...
                    <div class="card-panel dark:card-panel-dark">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="text-sm text-slate-500 dark:text-slate-400">{{ $request->blood_type ?? $request->blood_group ?? 'Unknown blood group' }}</p>
                            </div>
                            <span class="rounded-full bg-yellow-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-yellow-700 dark:bg-yellow-900 dark:text-yellow-200">{{ $request->status ?? 'Pending' }}</span>
                        </div>
                        <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Updated {{ optional($request->updated_at)->diffForHumans() }}</p>
                    </div>
                @empty
