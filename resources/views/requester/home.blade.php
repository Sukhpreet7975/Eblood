...
<div class="space-y-8">
            ...
            <div class="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] xl:items-start">
                ...
                <div class="space-y-3">
                    ...
                    <div class="rounded-[1rem] bg-white px-4 py-3 shadow-sm dark:bg-slate-950/50">
                        ...
                        <p class="mt-2 text-sm font-semibold text-slate-900 dark:text-white">{{ $queueHealthLabel }}</p>
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-300">{{ $queueHealthCopy }}</p>
                    </div>
                </div>
            </div>
    ...
    <section class="card-panel dark:card-panel-dark">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <a href="{{ route('requester.requests.index') }}" class="inline-flex w-full items-center justify-center rounded-full bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-700 sm:w-auto">View all requests</a>
        </div>
        ...
        <div class="mt-4 grid gap-3 xl:grid-cols-3">
            <div class="rounded-[1.25rem] border border-rose-100 bg-rose-50 px-4 py-4 dark:border-rose-900/60 dark:bg-rose-950/30">
                ...
                <p class="mt-3 text-sm font-semibold text-slate-900 dark:text-white">{{ $oldestPendingLabel }}</p>
                <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">{{ $oldestPendingDays }} days waiting • {{ optional($oldestPendingRequest)->blood_group ?? 'Unknown' }} • {{ optional($oldestPendingRequest)->hospital ?? 'Unknown' }}</p>
                @if($oldestPendingUrl)
                    <a href="{{ $oldestPendingUrl }}" class="mt-4 inline-flex items-center rounded-full bg-red-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-red-700">Open request</a>
                @endif
            </div>
            ...
            <div class="rounded-[1.25rem] border border-amber-100 bg-amber-50 px-4 py-4 dark:border-amber-900/70 dark:bg-amber-950/30">
                ...
                <p class="mt-3 text-2xl font-bold text-slate-900 dark:text-white">{{ $queueHealthLabel }}</p>
                <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">{{ $queueHealthCopy }}</p>
                <a href="{{ route('requester.requests.index') }}" class="mt-4 inline-flex items-center rounded-full bg-amber-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-amber-700">Review queue</a>
            </div>
            ...
            <div class="rounded-[1.25rem] border border-emerald-100 bg-emerald-50 px-4 py-4 dark:border-emerald-900/70 dark:bg-emerald-950/30">
                ...
                <a href="{{ route('requester.requests.create') }}" class="mt-4 inline-flex items-center rounded-full bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700">New request</a>
            </div>
        </div>
    </section>
            ...
            @forelse($recentRequests as $request)
                ...
                <div class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-300">
                        <span>Days waiting: {{ $requestAge }}</span>
                        <span>•</span>
                        <span>Submitted {{ optional($request->created_at)->diffForHumans() }}</span>
                    </div>
                    @if($requestUrl)
                        <div class="mt-4">
                            <a href="{{ $requestUrl }}" class="inline-flex items-center rounded-full bg-red-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-red-700">
                                View details
                            </a>
                        </div>
                    @endif
